function initTabNav() {
  ...
  if (dataClick.length && dataTarget.length) {
    ...
    dataClick.forEach(click => {
      click.addEventListener("click", tabNavegation);
    });
  };
};
...
function initAccordion() {
  const dtLista = document.querySelectorAll(".faq-lista dt"),
        className = "ativo";
  ...
  if (dtLista.length) {
    ...
    dtLista.forEach(dt => {
      dt.addEventListener("click", ativaAccordion);
    });
  };
};
